...
const Card3D = React.forwardRef<HTMLDivElement, Card3DProps>(
  (
    ...
    // Per-card idle "shuffle" — random amplitude / timing so the cards drift
    // out of sync when nothing is hovered.
    const idle = useMemo(() => {
    ...
    return (
      <motion.div
        ...
        animate={
          disabled
            ? { rotateX: 0, rotateY: 0, y: 0, z: 0 }
            : hovered
              ? { rotateX: mousePos.y, rotateY: mousePos.x, y: 0, z: 30 }
              : reduceMotion
                ? { rotateX: 0, rotateY: 0, y: 0, z: 0 }
                : {
                    rotateX: [0, idle.rx, 0, -idle.rx * 0.7, 0],
                    rotateY: [0, -idle.ry, 0, idle.ry * 0.8, 0],
                    y: [0, -idle.y, 0, idle.y * 0.4, 0],
                    z: 0,
                  }
        }
        transition={
          hovered || disabled || reduceMotion
      ...
      >
        ...
        >
          ...
          >
            <motion.h3
              className="text-xl font-semibold tracking-tight text-[#f7d7ff] drop-shadow-md"
              animate={{ scale: hovered ? 1.02 : 1 }}
              transition={{ duration: 0.3 }}
            >
              {title}
...
export type { CardData, Card3DProps, Card3DListProps };
