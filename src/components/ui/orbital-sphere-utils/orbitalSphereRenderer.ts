...
export function createOrbitalSphereRenderer(canvas: HTMLCanvasElement, getOptions: () => OrbitalSphereOptions) {
  ...
  return {
    resize(width: number, height: number) {
      ...
      if (width >= 1024) { networkGroup.position.set(0, 0, -2); responsiveScale = 1.15; camera.position.z = 5.5; }
      else { networkGroup.position.set(0, 0, -3); responsiveScale = 1; camera.position.z = 6.5; }
